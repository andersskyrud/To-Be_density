/** ANALYSER TIL ARTIKKEL 4 I TO-BE 1 **/
...
set scheme s1mono // penere grafer

// Tabell 1

table ScreeningType, c(p50 alder p25 alder p75 alder)
tab ScreeningType pre_sub, row
table ScreeningType, c(p50 BMI p25 BMI p75 BMI)
table ScreeningType, c(p50 BV_cm3 p25 BV_cm3 p75 BV_cm3)
table ScreeningType, c(p50 VBD p25 VBD p75 VBD)
table ScreeningType, c(p50 FGV_cm3 p25 FGV_cm3 p75 FGV_cm3)
tab ScreeningType VDG_5ed, row 
tab ScreeningType VBD_gr, row 
table VDG_5ed ScreeningType, c(min VBD max VBD)
table VBD_gr ScreeningType, c(min VBD max VBD)

// Figur 2a-f (for å fjerne marginene mellom kvintilene i grafene kan man legge til "style(compact)" uten anførselstegn som option til by())
...
preserve 
replace ScreeningType = 0 if ScreeningType == 2
label define VDG_5ed2 1 `""VDG 1*" "3929    3212""' 2 `""VDG 2*" "6216    6280""' 3 `""VDG 3" "3152     3655""' 4 `""VDG 4" " 962     1136""'
label value VDG_5ed VDG_5ed2
collapse (mean) meankonsensus = konsensus (sd) sdkonsensus = konsensus (count) n = konsensus, by(ScreeningType VDG_5ed)
gen prosentkonsensus = string(100 * meankonsensus, "%8.1f") + "%"
gen hikonsensus = meankonsensus + invttail(n-1,0.025)*(sdkonsensus / sqrt(n))
gen lokonsensus = meankonsensus - invttail(n-1,0.025)*(sdkonsensus / sqrt(n))
graph twoway (bar meankonsensus ScreeningType, fcolor(red%40)) (rcap hikonsensus lokonsensus ScreeningType, lcolor(red%40)) (scatter meankonsensus ScreeningType, ms(none) mlabel(prosentkonsensus) mlabpos(6)), ///
	by(VDG_5ed, title("A", span size(6)) subtitle("Volpara Density Grades") rows(1) legend(off) note("")) subtitle(,fcolor(white) bcolor(white)) ///
	yscale(range(0 0.1)) xlabel(0 "DBT" 1 "DM", noticks) xtitle("Screening technique") ytitle("Consensus (%)") ///
	ylabel(0(0.02)0.1 0 "0" 0.02 "2" 0.04 "4" 0.06 "6" 0.08 "8" 0.1 "10", grid angle(h))  plotregion(color(white)) graphregion(color(white))
graph save figurkonsensus.gph, replace
restore 

// Appendix 2: Table 1 - Legger til koder for Z-tester for å kunne lese av tallene på konfidensintervallene

// VBD-kvintiler

foreach var of varlist konsensus FP recall biopsi SDC {
	prtest `var' if VBD_gr == 0, by(ScreeningType)
	}
foreach var of varlist konsensus FP recall biopsi SDC {
	prtest `var' if VDG_5ed == 1, by(ScreeningType)
	}
foreach var of varlist konsensus FP recall biopsi SDC {
	prtest `var' if VBD_gr == 2, by(ScreeningType)
	}
foreach var of varlist konsensus FP recall biopsi SDC {
	prtest `var' if VBD_gr == 3, by(ScreeningType)
	}
foreach var of varlist konsensus FP recall biopsi SDC {
	prtest `var' if VBD_gr == 4, by(ScreeningType)
	}

prtesti 54 13 90 11, c 
prtesti 36 13 38 11, c 
prtesti 72 12 110 16, c 
prtesti 35 12 58 16, c 
prtesti 87 19 137 23, c 
prtesti 42 19 60 23, c 
prtesti 107 30 116 23, c 
prtesti 59 30 58 23, c 
prtesti 120 21 113 13, c
prtesti 76 21 55 13, c 
...
prtesti 48 18 46 15, c // VDG 1 - PPV-2
prtesti 200 48 267 39, c // VDG 2 - PPV-1
prtesti 103 48 129 39, c // VDG 2 - PPV-2
prtesti 129 23 147 25, c // VDG 3 - PPV-1
prtesti 78 23 69 25, c // VDG 3 - PPV-2
prtesti 30 6 46 7, c // VDG 4 - PPV-1
prtesti 19 6 25 7, c // VDG 4 - PPV-2 

// VBD_tokat

foreach var of varlist konsensus FP recall biopsi SDC {
	prtest `var' if VBD_tokat == 0, by(ScreeningType)
	}
foreach var of varlist konsensus FP recall biopsi SDC {
	prtest `var' if VBD_tokat == 1, by(ScreeningType)
	}
	
table recall VBD_tokat ScreeningType
table biopsi VBD_tokat ScreeningType
table SDC VBD_tokat ScreeningType

prtesti 213 44 337 50, c // VBD_tokat 0 - PPV-1
prtesti 113 44 156 50, c // VBD_tokat 0 - PPV-2
prtesti 227 51 229 36, c // VBD_tokat 1 - PPV-1
prtesti 135 51 113 36, c // VBD_tokat 1 - PPV-2

// VDG_tokat

foreach var of varlist konsensus FP recall biopsi SDC {
	prtest `var' if VDG_tokat == 0, by(ScreeningType)
	}
foreach var of varlist konsensus FP recall biopsi SDC {
	prtest `var' if VDG_tokat == 1, by(ScreeningType)
	}
	
table recall VDG_tokat ScreeningType
table biopsi VDG_tokat ScreeningType
table SDC VDG_tokat ScreeningType

prtesti 281 66 373 54, c // VDG_tokat 0 - PPV-1
prtesti 151 66 175 54, c // VDG_tokat 0 - PPV-2
prtesti 159 29 193 32, c // VDG_tokat 1 - PPV-1
prtesti 97 29 94 32, c // VDG_tokat 1 - PPV-2

// Tabell 2 og 3 - tumorkarakteristikk for invasive

// Bytter ut tumorstørrelsesvariabelen fra artikkel 2 med Hildegunns nye måling etter regranskning
...
// Tabell 4 - RR for SDC og recall etter tetthet med BV som kontinuerlig - først ujusterte RR og så justert for alle variablene 

glm SDC i.VDG_5ed if ScreeningType == 1, family(binomial) link(log) eform
glm SDC i.alder_gr if ScreeningType == 1, family(binomial) link(log) eform
glm SDC i.pre_sub if ScreeningType == 1, family(binomial) link(log) eform
glm SDC BV_cm3 if ScreeningType == 1, family(binomial) link(log) eform
glm SDC i.VDG_5ed i.alder_gr i.pre_sub BV_cm3 if ScreeningType == 1, family(binomial) link(log) eform
glm SDC i.VDG_5ed if ScreeningType == 2, family(binomial) link(log) eform
glm SDC i.alder_gr if ScreeningType == 2, family(binomial) link(log) eform
glm SDC i.pre_sub if ScreeningType == 2, family(binomial) link(log) eform
glm SDC BV_cm3 if ScreeningType == 2, family(binomial) link(log) eform
glm SDC i.VDG_5ed i.alder_gr i.pre_sub BV_cm3 if ScreeningType == 2, family(binomial) link(log) eform

glm recall i.VDG_5ed if ScreeningType == 1, family(binomial) link(log) eform
glm recall i.alder_gr if ScreeningType == 1, family(binomial) link(log) eform
glm recall i.pre_sub if ScreeningType == 1, family(binomial) link(log) eform
glm recall BV_cm3 if ScreeningType == 1, family(binomial) link(log) eform
glm recall i.VDG_5ed i.alder_gr i.pre_sub BV_cm3 if ScreeningType == 1, family(binomial) link(log) eform
glm recall i.VDG_5ed if ScreeningType == 2, family(binomial) link(log) eform
glm recall i.alder_gr if ScreeningType == 2, family(binomial) link(log) eform
glm recall i.pre_sub if ScreeningType == 2, family(binomial) link(log) eform
glm recall BV_cm3 if ScreeningType == 2, family(binomial) link(log) eform
glm recall i.VDG_5ed i.alder_gr i.pre_sub BV_cm3 if ScreeningType == 2, family(binomial) link(log) eform

glm FP i.VDG_5ed if ScreeningType == 1, family(binomial) link(log) eform
glm FP i.alder_gr if ScreeningType == 1, family(binomial) link(log) eform
glm FP i.pre_sub if ScreeningType == 1, family(binomial) link(log) eform
glm FP BV_cm3 if ScreeningType == 1, family(binomial) link(log) eform
glm FP i.VDG_5ed i.alder_gr i.pre_sub BV_cm3 if ScreeningType == 1, family(binomial) link(log) eform
glm FP i.VDG_5ed if ScreeningType == 2, family(binomial) link(log) eform
glm FP i.alder_gr if ScreeningType == 2, family(binomial) link(log) eform
glm FP i.pre_sub if ScreeningType == 2, family(binomial) link(log) eform
glm FP BV_cm3 if ScreeningType == 2, family(binomial) link(log) eform
glm FP i.VDG_5ed i.alder_gr i.pre_sub BV_cm3 if ScreeningType == 2, family(binomial) link(log) eform

// Appendix 4-7 Sensitivitetsanalyse
...
// Figur splines-graf for SDC over VBD 

mkspline VBDcubDM = VBD if ScreeningType == 1, cubic nknots(4) // lager kubiske splines, antall knuter gir lavest AIC i nedenstående modeller

glm SDC VBDcubDM* i.alder_gr i.pre_sub BV_cm3, family(binomial) link(logit) eform // modellen er en logit-modell fordi den kan jeg "predicte"
estat ic // dette gjør at man får opp to mål på hvor god modellen er - AIC og BIC

predictnl xb1 = _b[_cons] + _b[VBDcubDM1]*VBDcubDM1 + _b[VBDcubDM2]*VBDcubDM2 + _b[VBDcubDM3]*VBDcubDM3, se(se1) // predicter (og dermed genererer) to parametere fra modellen: xb og se 

gen lb_xb1 = xb1 - invnormal(0.975)*se1
gen ub_xb1 = xb1 + invnormal(0.975)*se1 

gen lb1 = exp(lb_xb1)/(1+exp(lb_xb1))
gen ub1 = exp(ub_xb1)/(1+exp(ub_xb1)) // lager øvre og nedre konfidensintervall

gen or1 = exp(xb1)
gen p1 = or1 / (1 + or1) // lager punktestimatet 

mkspline VBDcubDBT = VBD if ScreeningType == 2, cubic nknots(4) // alt under er samme prosedyre for den andre screeningteknikken 
...
drop VBDcubDM1-p2 // fjerne de genererte variablene så jeg kan gjøre alt på nytt med et annet utfallsmål 

// Figur splines-graf for recall over VBD 
...
// Figur splines-graf for SDC over VBD - CRUDE

mkspline VBDcubDM = VBD if ScreeningType == 1, cubic nknots(4) // lager kubiske splines, antall knuter gir lavest AIC i nedenstående modeller

glm SDC VBDcubDM*, family(binomial) link(logit) eform // modellen er en logit-modell fordi den kan jeg "predicte"
estat ic // dette gjør at man får opp to mål på hvor god modellen er - AIC og BIC
...
// Figur splines-graf for recall over VBD - CRUDE 
